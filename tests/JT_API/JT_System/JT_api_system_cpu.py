...
def JT_api_system_cpu_get():
    ...
    for _type in ["percent", "times", "times_percent", "count", "stats", "frequency", "load"]:
        Assertion.contain(response.json(), _type, f"API system cpu returned wrong response ({_type} not included)")
...
def JT_api_system_cpu_get_per_category():
    ...
    for _type in ["percent", "times", "times_percent", "count", "stats", "frequency", "load"]:
        ...
        if response.status_code != 200:
            ...
            Assertion(False, f"API system cpu returned wrong status code for type '{_type}'")
            continue

        Assertion.eq(list(response.json().keys())[0], _type, f"API system cpu returned wrong response for type '{_type}'")
