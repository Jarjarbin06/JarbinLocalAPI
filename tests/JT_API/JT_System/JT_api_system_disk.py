...
def JT_api_system_disk_get():
    ...
    for _type in ["usage", "partitions", "io"]:
        Assertion.contain(response.json()["data"], _type, f"API system disk returned wrong response ({_type} not included)")
...
def JT_api_system_disk_get_per_category():
    ...
    for _type in ["usage", "partitions", "io"]:
        ...
        Assertion.eq(list(response.json()["data"].keys())[0], _type, f"API system disk returned wrong response for type '{_type}'")
