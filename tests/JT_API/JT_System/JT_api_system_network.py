...
def JT_api_system_network_get():
    ...
    for _type in ["interfaces", "interface_status", "io", "connections"]:
        Assertion.contain(response.json(), _type, f"API system network returned wrong response ({_type} not included)")
...
def JT_api_system_network_get_per_category():
    ...
    for _type in ["interfaces", "interface_status", "io", "connections"]:
        ...
        if response.status_code != 200:
            ...
            Assertion(False, f"API system network returned wrong status code for type '{_type}'")
            continue

        Assertion.eq(list(response.json().keys())[0], _type, f"API system network returned wrong response for type '{_type}'")
