...
def JT_api_system_processes_get():
    ...
    for _type in ["count", "pids", "processes"]:
        Assertion.contain(response.json()["data"], _type, f"API system processes returned wrong response ({_type} not included)")
...
def JT_api_system_processes_get_per_category():
    ...
    for _type in ["count", "pids", "processes"]:
        ...
        Assertion.eq(response.status_code, 200, f"API system processes returned wrong status code for type '{_type}'")
        Assertion.eq(list(response.json()["data"].keys())[0], _type, f"API system processes returned wrong response for type '{_type}'")
